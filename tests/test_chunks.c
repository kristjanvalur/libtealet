...
int main(void) {
  ...
  printf("Summary: %zu tealets sharing %zu stacks (%zu chunks total) = %.2fx "
         "expansion\n",
         (size_t)stats.n_active - 1, /* Exclude main */
         stats.stack_count, stats.stack_chunk_count, (double)stats.stack_bytes_expanded / (double)stats.stack_bytes);
