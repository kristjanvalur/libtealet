 ...
 */
static int run_integrity_switch_case(int fail_policy, int write_inside, int *first_result, int *recovery_result) {
  tealet_t *main_tealet;
  tealet_t *writer;
  tealet_config_t cfg = TEALET_CONFIG_INIT;
 ...
 * write_guard_page=1 targets guarded page (hard-fault path, usually
 * subprocess).
 */
static int run_mprotect_split_case(int write_guard_page, int *first_result, int *recovery_result,
  ...
  cfg.flags = TEALET_CONFIGF_STACK_INTEGRITY | TEALET_CONFIGF_STACK_SNAPSHOT | TEALET_CONFIGF_STACK_GUARD;
...
static int run_mprotect_split_case_with_stack_limit(int write_guard_page, int *first_result, int *recovery_result) {
  int stack_limit_marker;

  return run_mprotect_split_case(write_guard_page, first_result, recovery_result, &stack_limit_marker);
}
...
static void test_set_canonicalizes_unsupported(void) {
  ...
  set_cfg.flags = TEALET_CONFIGF_STACK_INTEGRITY | TEALET_CONFIGF_STACK_GUARD | TEALET_CONFIGF_STACK_SNAPSHOT;
...
static void test_snapshot_soft_error_inside_watch_range(void) {
  ...
  assert(run_integrity_switch_case(TEALET_STACK_INTEGRITY_FAIL_ERROR, 1, &first_result, &recovery_result) == 0);
...
static void test_snapshot_write_outside_watch_range_ok(void) {
  ...
  recovery_result = TEALET_ERR_INTEGRITY;
  assert(run_integrity_switch_case(TEALET_STACK_INTEGRITY_FAIL_ERROR, 0, &first_result, &recovery_result) == 0);
...
static void test_mprotect_snapshot_prefix_soft_error(void) {
  ...
  if (child_exit == 20) {
    printf("  SKIPPED (snapshot prefix not representable for this stack "
           "alignment)\n");
    test_passed++;
    return;
  }
  if (child_exit == 21) {
    printf("  SKIPPED (active stack pointer lies inside computed guard "
           "interval)\n");
    test_passed++;
    return;
  }
...
static void test_mprotect_guard_page_segv_subprocess(void) {
  ...
  if (WIFEXITED(wait_status)) {
    child_exit = WEXITSTATUS(wait_status);
    if (child_exit == 112) {
      printf("  SKIPPED (guard page was not enforceable for this runtime stack "
             "layout)\n");
      fflush(stdout);
      test_passed++;
      return;
